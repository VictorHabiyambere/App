            ...
            while (true) {
                ...
                if (!ref.shutdown.get()) {
                    ...
                    if (ManagedChannelOrphanWrapper.logger.isLoggable(level)) {
                        LogRecord lr = new LogRecord(level, "*~*~*~ Previous channel {0} was not shutdown properly!!! ~*~*~*" + System.getProperty("line.separator") + "    Make sure to call shutdown()/shutdownNow() and wait until awaitTermination() returns true.");
